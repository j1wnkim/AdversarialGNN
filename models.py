...
class KProp(MessagePassing):
    def __init__(self, steps, aggregator, add_self_loops, normalize, cached, transform=torch.nn.Softmax(dim=1)):
...
class NodeClassifier(torch.nn.Module):
    ...
    def forward(self, data):
        # data.adj_t for data.edge_index
        x = data.x 
        ...
        p_yt_x = self.y_prop(p_yp_x, data.edge_index)                                       # P(y~|x')
    ...
    def training_step(self, data):
        ...
        if self.cached_yt is None:
            ...
            self.cached_yt = self.y_prop(yp, data.edge_index)  # y~
    ...
    def validation_step(self, data):
        ...
        loss = self.cross_entropy_loss(p_yp_x[data.val_mask], data.y[data.val_mask]).item()
