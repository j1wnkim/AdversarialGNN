...
from attacks import AttackMode
import math
...
class FeaturePerturbation: ### The actual attack that is imposed 
    def __init__(self,
...
class FeaturePerturbationAttack: ### The actual attack that is imposed 
    def __init__(self,
                 mechanism:     dict(help='feature perturbation mechanism', choices=list(supported_feature_mechanisms),
                                     option='-m') = 'mbm',
                 x_eps:         dict(help='privacy budget for feature perturbation', type=float,
...
#         Attack below
        ...
        n, d = data.x.size()
        m = d-1
        data.x = data.x + ( self.input_range[0] - self.input_range[1] )/( math.exp(self.x_eps / m) - 1 )
#         Attack above
        data.x = supported_feature_mechanisms[self.mechanism](
            eps=self.x_eps,
            input_range=self.input_range
