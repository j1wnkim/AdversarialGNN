...
class FeatureTransform:
    ...
    def __init__(self, feature: dict(help='feature transformation method', choices=supported_features, option='-f') = 'raw'): # type: ignore
...
class LabelPerturbation:
    def __init__(self,
                 y_eps: dict(help='privacy budget for label perturbation', # type: ignore 
                             type=float, option='-ey') = np.inf):
        self.y_eps = y_eps

    def __call__(self, data):
        num_classes = len(np.unique(data.y.cpu().numpy()))
        data.y = F.one_hot(data.y, num_classes=num_classes)
        ...
        if not np.isinf(self.y_eps):
            mechanism = RandomizedResopnse(eps=self.y_eps, d=num_classes)
        ...
        data.T = torch.ones(num_classes, num_classes, device=data.y.device) * p_ij
